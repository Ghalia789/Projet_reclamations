        ...
        <table class="w-full border-collapse bg-white text-left text-sm">
          ...
          <tbody>
            ...
            <tr *ngIf="produits.length === 0">
              <td colspan="3" class="px-4 py-5 text-center text-slate-500">Aucun produit disponible.</td>
            </tr>
          </tbody>
        </table>
      </div>
...
export class ProduitsComponent implements OnInit {
  ...
  submit(): void {
    ...
    this.produitService.create(this.form.getRawValue()).subscribe({
      next: () => {
        this.form.reset({ nom: '', categorie: '' });
        this.isSubmitting = false;
        this.submitMessage = 'Produit ajoute avec succes.';
        this.loadProduits();
      },
      error: () => {
        this.isSubmitting = false;
